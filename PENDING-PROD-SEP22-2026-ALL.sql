-- #####################################################################################
--  NIZAMI FARMS — PROD SQL BATCH                                        22-Sep-2026
--  ONE file. Run it ONCE, BEFORE uploading the web files, then /api/public/xclean,
--  then build the PRIMARY APK.
--
--  It carries BOTH of today's rounds, because they ship together (routes/web.php and
--  routes/api.php each contain both):
--      SECTION 1 — TILL KEEPER      (entered_by, cash counts, the cash pill)
--      SECTION 2 — FROZEN RECIPES   (recipes, ingredients, receipt capture, costing)
--
--  ⚠⚠ ALTER TABLE is NOT rolled back inside a transaction. Do NOT wrap this in one.
--     Every statement below is guarded, so the whole file is SAFE TO RE-RUN: a part
--     that is already applied prints an "already ..." note instead of erroring.
--
--  HOW TO RUN (phpMyAdmin): paste one SECTION at a time and read the notes it prints
--  before moving on. Running the whole file in one go also works.
--
--  Nothing here drops a column, rewrites a balance or touches money. Section 1 adds
--  4 additive objects + 1 backfill + 2 seed UPDATEs; Section 2 adds 6 tables, 4
--  nullable columns, 2 permissions and a catalogue tagging pass.
-- #####################################################################################


-- #####################################################################################
--  SECTION 1 — TILL KEEPER
--  "Who touched my cash" + the daily count checkpoint.
-- #####################################################################################

-- -------------------------------------------------------------------------------------
-- 1.0  PRE-FLIGHT — read only. Run these first and read the answers.
-- -------------------------------------------------------------------------------------
SELECT 'pre-flight 1a: entered_by already present' AS check_name, COUNT(*) AS found
  FROM information_schema.columns
 WHERE table_schema = DATABASE() AND table_name = 't_fin_ledger' AND column_name = 'entered_by';
-- EXPECT 0 on a first run, 1 on a re-run.

SELECT 'pre-flight 1b: till tables already present' AS check_name, COUNT(*) AS found
  FROM information_schema.tables
 WHERE table_schema = DATABASE()
   AND table_name IN ('t_fin_ledger_watch','t_fin_cash_count');
-- EXPECT 0 on a first run, 2 on a re-run.

SELECT 'pre-flight 1c: the two keepers exist' AS check_name, u.id AS user_id, u.fullname,
       a.id AS account_id, a.account_code, a.account_category
  FROM t_fin_account_users au
  JOIN t_sys_user u     ON u.id = au.user_id
  JOIN t_fin_accounts a ON a.id = au.account_id
 WHERE (a.account_code = 'NF_CASH' AND u.fullname = 'Shabib')
    OR (a.account_code = 'ONLINE'  AND u.fullname = 'Taimur');
-- EXPECT EXACTLY 2 ROWS. Steps 1.6 and 1.7 are UPDATEs — they set is_keeper on rows that
-- must already exist. If a row is missing here, tag that person on that account in the
-- Hub's account-users panel FIRST, or the seed silently does nothing and that person is
-- never asked to count and never gets the pill.

-- -------------------------------------------------------------------------------------
-- 1.1  WHOSE HAND POSTED THE ROW.
--      t_fin_ledger.created_by is the REQUESTER on expense rows
--      (LedgerPostingService: 'created_by' => $request->requester_user_id), so it names
--      the person the expense belongs to, not who pressed the button. entered_by is the
--      logged-in actor, stamped by LedgerModel::creating.
--      NULL = console / system / pre-migration row -> reads as "System".
-- -------------------------------------------------------------------------------------
SET @sql := (SELECT IF(
    (SELECT COUNT(*) FROM information_schema.columns
      WHERE table_schema = DATABASE() AND table_name = 't_fin_ledger'
        AND column_name = 'entered_by') = 0,
    'ALTER TABLE `t_fin_ledger`
        ADD COLUMN `entered_by` INT(11) NULL DEFAULT NULL COMMENT "the logged-in actor who posted this row" AFTER `created_by`',
    'SELECT "1.1: t_fin_ledger already has entered_by" AS note'));
PREPARE s FROM @sql; EXECUTE s; DEALLOCATE PREPARE s;

SET @sql := (SELECT IF(
    (SELECT COUNT(*) FROM information_schema.statistics
      WHERE table_schema = DATABASE() AND table_name = 't_fin_ledger'
        AND index_name = 'idx_ledger_entered_by') = 0,
    'CREATE INDEX `idx_ledger_entered_by` ON `t_fin_ledger` (`entered_by`)',
    'SELECT "1.1b: index idx_ledger_entered_by already present" AS note'));
PREPARE s FROM @sql; EXECUTE s; DEALLOCATE PREPARE s;

-- -------------------------------------------------------------------------------------
-- 1.2  BACKFILL the actor from the audit log (ledger audit starts 2026-07-04).
--      t_sys_audit_log.user_id on a 'created' row IS the logged-in actor. Rows older
--      than the audit log keep entered_by NULL and fall back to created_by when read.
--      Re-runnable: it only ever fills NULLs.
-- -------------------------------------------------------------------------------------
...
-- EXPECT ~6,300 rows matched on the first run, 0 on a re-run.

SELECT '1.2: rows now carrying an actor' AS check_name, COUNT(*) AS with_actor
  FROM t_fin_ledger WHERE entered_by IS NOT NULL;

-- -------------------------------------------------------------------------------------
-- 1.3  THE CASH PILL's unread watermark. One row per person, upserted by the app.
--      Everything newer than last_seen_ledger_id that is not their own hand is "unread".
-- -------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `t_fin_ledger_watch` (
  ...
  COMMENT='Cash-pill read watermark, one row per person.';

-- -------------------------------------------------------------------------------------
-- 1.4  THE TILL COUNT — the keeper's own figure, and the SEAL.
--
...
--      higher id arrived AFTER the count; if it is also dated on/before the count day,
--      it rewrote history behind the keeper's back and the ledger says so.
--
--      counted_at is DATETIME, matching t_ops_attendance.cash_confirmed_at, written by
--      PHP in Asia/Karachi. Prod runs one clock so `l.updated_at > c.counted_at`
--      (TIMESTAMP vs DATETIME) is exact there. On the DEV REPLICA only, add
--      `SET SESSION time_zone='+01:00';` before reading or the drift columns read 2h out.
--
...
CREATE TABLE IF NOT EXISTS `t_fin_cash_count` (
  ...
  COMMENT='A keeper counting the till: his figure, the books figure, and the seal.';

-- -------------------------------------------------------------------------------------
-- 1.5  WHO HOLDS THE CASH. Deliberately NOT is_default — that means "his default
--      payment source", which Taimur legitimately is on his own accounts.
--      Counting is only offered on account_category = 'cash'; WATCHING also covers bank.
-- -------------------------------------------------------------------------------------
SET @sql := (SELECT IF(
    (SELECT COUNT(*) FROM information_schema.columns
      WHERE table_schema = DATABASE() AND table_name = 't_fin_account_users'
        AND column_name = 'is_keeper') = 0,
    'ALTER TABLE `t_fin_account_users`
        ADD COLUMN `is_keeper` TINYINT(1) NOT NULL DEFAULT 0 COMMENT "holds / answers for this account" AFTER `is_default`',
    'SELECT "1.5: t_fin_account_users already has is_keeper" AS note'));
PREPARE s FROM @sql; EXECUTE s; DEALLOCATE PREPARE s;

-- -------------------------------------------------------------------------------------
-- 1.6  SEED: Shabib holds NF Cash (Main Till) — counted AND watched.
...
-- -------------------------------------------------------------------------------------
-- 1.7  SEED: Taimur answers for the ONLINE account — watched, NEVER counted.
--      A BANK keeper is told when somebody else moves the account (Shabib's transfers
--      out, online salaries) but is never asked to count it; counting is cash-only.
--      See TillCountService::keeperAccounts.
-- -------------------------------------------------------------------------------------
UPDATE `t_fin_account_users`
   SET `is_keeper` = 1
...
-- -------------------------------------------------------------------------------------
-- 1.8  VERIFY SECTION 1 — expect exactly 2 rows: Shabib/NF_CASH, Taimur/ONLINE.
-- -------------------------------------------------------------------------------------
SELECT '1.8: keepers' AS check_name, u.fullname, a.account_code, a.account_category
  FROM t_fin_account_users au
  JOIN t_sys_user u     ON u.id = au.user_id
  JOIN t_fin_accounts a ON a.id = au.account_id
...
-- #####################################################################################
--  SECTION 2 — FROZEN RECIPES · RECEIPT CAPTURE · PURCHASE-TO-BATCH COSTING
--  Verbatim copy of database/migrations/frozen_recipes_costing_sep22_2026.sql.
-- #####################################################################################
...
-- #####################################################################################
--  END OF BATCH. Next steps, in order:
--    1. Upload app/ + resources/ + routes/  (42 files, both rounds)
--    2. Hit /api/public/xclean            (REQUIRED - both route files changed)
--    3. Build the PRIMARY APK             (build-production-apk-auto.bat, option 1)
--    4. Anyone given manage_khaas_recipes / view_khaas_costing logs out and in once.
-- #####################################################################################
