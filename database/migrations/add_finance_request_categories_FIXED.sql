-- =====================================================
-- Add Finance Request Categories - FIXED
-- =====================================================
...
-- =====================================================
-- NOTE: Uses 'color_class' instead of 'color' to match actual schema
-- =====================================================

-- Check if categories already exist before inserting
INSERT INTO t_req_category (category_code, category_name, description, icon, color_class, is_active, created_at)
SELECT * FROM (
    SELECT 
...
SELECT * FROM (
    SELECT 
        ...
        '💸' as icon,
        'bg-red-100 text-red-800' as color_class,
        1 as is_active,
        NOW() as created_at
) AS tmp
WHERE NOT EXISTS (
...
SELECT * FROM (
    SELECT 
        ...
        '🔄' as icon,
        'bg-purple-100 text-purple-800' as color_class,
        1 as is_active,
        NOW() as created_at
) AS tmp
WHERE NOT EXISTS (
...
SELECT * FROM (
    SELECT 
        ...
        '📄' as icon,
        'bg-green-100 text-green-800' as color_class,
        1 as is_active,
        NOW() as created_at
) AS tmp
WHERE NOT EXISTS (
