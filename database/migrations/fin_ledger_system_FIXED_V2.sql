-- ========================================
-- FINANCIAL LEDGER SYSTEM - FIXED V2
-- ========================================
-- This version handles the t_crm_prod_order FK issue
-- Made order_id BIGINT to match potential BIGINT id in orders table
...
-- ----------------------------------------
-- NOTE: order_id is BIGINT to match potential BIGINT in t_crm_prod_order
CREATE TABLE t_fin_ledger (
    ...
    order_id BIGINT NULL COMMENT 'FK to t_crm_prod_order(id) - BIGINT to match orders table',
...
SELECT '✓ Created t_fin_ledger (order_id is BIGINT)' as Status;
...
-- FKs for t_fin_ledger -> t_crm_prod_order (TRY with error handling)
-- This might fail if t_crm_prod_order doesn't exist or has different structure
-- We'll continue even if it fails
SET @order_fk_sql = '
    ALTER TABLE t_fin_ledger
...
-- Try to add the FK, but don't fail if it doesn't work
...
BEGIN
    ...
    BEGIN
        SELECT '⚠ WARNING: Could not add FK to t_crm_prod_order (table may not exist or ID type mismatch)' as Status;
    END;
    
    PREPARE stmt FROM @order_fk_sql;
    EXECUTE stmt;
    DEALLOCATE PREPARE stmt;
    SELECT '✓ FK: t_fin_ledger -> t_crm_prod_order' as Status;
END;
...
SELECT 'Next: Create AccountModel, LedgerModel, VendorModel' as '';
SELECT '========================================' as '';
