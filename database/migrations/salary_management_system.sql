-- =====================================================================
-- SALARY MANAGEMENT SYSTEM - Complete Implementation
-- =====================================================================
...
-- =====================================================================
-- This stores salary configuration for ANY employee (riders, managers, office staff)
-- Separate from t_ops_rider_profile so non-riders can also have salaries

CREATE TABLE IF NOT EXISTS t_hr_employee_profile (
    ...
    -- Status
    is_active BOOLEAN DEFAULT TRUE COMMENT 'Is salary profile active?',
    ...
    -- Foreign Key
    CONSTRAINT fk_hr_profile_user FOREIGN KEY (user_id) REFERENCES t_sys_user(id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS t_hr_employee_loans (
    id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    -- Ledger integration (if loan was disbursed via cash)
    ...
    CONSTRAINT fk_loan_created_by FOREIGN KEY (created_by) REFERENCES t_sys_user(id) ON DELETE SET NULL,
    CONSTRAINT fk_loan_cancelled_by FOREIGN KEY (cancelled_by) REFERENCES t_sys_user(id) ON DELETE SET NULL
...
CREATE TABLE IF NOT EXISTS t_hr_salary_slips (
    id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    late_deduction_overridden BOOLEAN DEFAULT FALSE COMMENT 'Was late deduction waived?',
    overtime_overridden BOOLEAN DEFAULT FALSE COMMENT 'Was overtime manually adjusted?',
    absent_deduction_overridden BOOLEAN DEFAULT FALSE COMMENT 'Was absent deduction adjusted?',
    loan_installment_skipped BOOLEAN DEFAULT FALSE COMMENT 'Was loan installment skipped?',
    has_manual_adjustments BOOLEAN DEFAULT FALSE COMMENT 'Any manual adjustments made?',
    ...
    CONSTRAINT fk_slip_created_by FOREIGN KEY (created_by) REFERENCES t_sys_user(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS t_hr_loan_payments (
    id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    -- Foreign Keys
    CONSTRAINT fk_loan_payment_loan FOREIGN KEY (loan_id) REFERENCES t_hr_employee_loans(id) ON DELETE CASCADE,
    CONSTRAINT fk_loan_payment_slip FOREIGN KEY (salary_slip_id) REFERENCES t_hr_salary_slips(id) ON DELETE SET NULL
...
-- =====================================================================
-- STEP 5: Add Salary Advance Category to Request System
-- =====================================================================
...
SELECT '✓ Step 5a: Added salary_advance category' as Status;
...
SELECT '✓ Step 5b: Configured approval levels for salary_advance (L1 + L2 required)' as Status;

-- =====================================================================
-- STEP 6: Add New Ledger Accounts for Salary Management
-- =====================================================================
...
SELECT '✓ Step 6a: Added EXPENSE_SALARY account' as Status;
...
SELECT '✓ Step 6b: Added ASSET_EMPLOYEE_LOANS account' as Status;

-- =====================================================================
-- STEP 7: Add Permissions for Salary Management
-- =====================================================================
...
SELECT '✓ Step 7: Added salary management permissions' as Status;
...
-- Check t_hr_employee_profile
SELECT '--- 1. t_hr_employee_profile Structure ---' as '';
DESCRIBE t_hr_employee_profile;

-- Check t_hr_employee_loans
SELECT '--- 2. t_hr_employee_loans Structure ---' as '';
DESCRIBE t_hr_employee_loans;

-- Check t_hr_salary_slips
SELECT '--- 3. t_hr_salary_slips Structure ---' as '';
DESCRIBE t_hr_salary_slips;

-- Check t_hr_loan_payments
SELECT '--- 4. t_hr_loan_payments Structure ---' as '';
DESCRIBE t_hr_loan_payments;

-- Check request category
SELECT '--- 5. Salary Advance Category ---' as '';
SELECT 
    c.id,
    c.category_code,
    c.category_name,
...
-- Check new accounts
SELECT '--- 6. Salary Management Accounts ---' as '';
SELECT 
    account_code,
    account_name,
    account_type,
...
-- Check permissions
SELECT '--- 7. Salary Management Permissions ---' as '';
SELECT 
    perm_key,
    perm_name,
    perm_desc
...
-- Check Foreign Keys
SELECT '--- 8. Foreign Key Constraints ---' as '';
SELECT 
    TABLE_NAME,
    CONSTRAINT_NAME,
    COLUMN_NAME,
...
SELECT '  1. t_hr_employee_profile - Employee salary configurations' as '';
SELECT '  2. t_hr_employee_loans - Loan tracking' as '';
SELECT '  3. t_hr_salary_slips - Monthly salary slips' as '';
SELECT '  4. t_hr_loan_payments - Loan payment history' as '';
SELECT '' as '';
SELECT 'Request Categories:' as '';
SELECT '  - salary_advance (L1 + L2 approval)' as '';
SELECT '' as '';
SELECT 'Ledger Accounts:' as '';
SELECT '  - EXPENSE_SALARY (Salary expense account)' as '';
SELECT '  - ASSET_EMPLOYEE_LOANS (Loans receivable)' as '';
SELECT '' as '';
SELECT 'Permissions Added: 7 permissions for salary management' as '';
SELECT '' as '';
SELECT 'Status: Ready for backend implementation!' as '';
SELECT '========================================' as '';
...
-- =====================================================================
-- 1. Create Models:
--    - app/Models/HR/EmployeeProfileModel.php
--    - app/Models/HR/EmployeeLoanModel.php
--    - app/Models/HR/SalarySlipModel.php
--    - app/Models/HR/LoanPaymentModel.php
--
-- 2. Create Services:
...
--
-- 5. Create Views:
--    - resources/views/pages/hr/...
--
-- 6. Update RequestModel to handle salary_advance post-approval
-- =====================================================================
