...
<div class="container-fixed">
    <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
        <div class="flex flex-col justify-center gap-2">
            <h1 class="text-xl font-semibold leading-none text-foreground">Create New Coupon</h1>
        ...
        <div class="card-body">
            <form action="{{ route('coupons.store') }}" method="POST">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="discount_type" class="select @error('discount_type') select-error @enderror" required>
                                ...
                                <option value="shipping" {{ old('discount_type') == 'shipping' ? 'selected' : '' }}>Free Shipping</option>
                            </select>
                            @error('discount_type')
                                <div class="form-hint text-danger">{{ $message }}</div>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="value_type" class="select @error('value_type') select-error @enderror" required>
                                ...
                                <option value="fixed_amount" {{ old('value_type') == 'fixed_amount' ? 'selected' : '' }}>Fixed Amount</option>
                            </select>
                            @error('value_type')
                                <div class="form-hint text-danger">{{ $message }}</div>
                            @enderror
                ...
                <div class="flex justify-end gap-3 mt-8">
                    <a href="{{ route('coupons.index') }}" class="kt-btn kt-btn-light">Cancel</a>
                    <button type="submit" class="kt-btn kt-btn-primary">Create Coupon</button>
                </div>
            </form>
        </div>
    </div>
